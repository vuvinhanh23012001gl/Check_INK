...
class HandlerWorkDetect:
    def __init__(self,obj_folder:Folder,Choose_Product : ChooseProduct,Manager_Product : ProductManager, obj_handler_calibration : HandlerCalibration):
    ...
    def Init(self):
        ...
        status_Check_file,message = self.obj_folder.check_file(PATH_PRODUCT_MODEL)
        ...
        while self.open_thread_inference:
            if self.start_detect:
                ...
                for index in range(0,self.count):
                    frame = obj_queue.get_timeout(name_queue_process_capture,10) # Sau 10s mà không nhận được ảnh thì cho ảnh lỗi và quay về lại gốc
    ...
    def worker_judget(self, index, arr_regualtion, img):
            ...
            if final_status is None:
                data_send = {datatype_Home:{
                    "index": index,
                    "arr_line": arr_line,
                    "img": img_base64,"judment_frame":status_judment
                }}
                obj_queue.put(name_queue_data_client, data_send)
                return
